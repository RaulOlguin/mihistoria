...
describe("Página Somos - Verificación de contenido", () => {
  ...
  test("muestra la información de Cristopher Osses", () => {
    ...
    const cristopherSection = screen.getByRole("heading", { name: /Cristopher Osses/i }).closest(".descripcion");

    // Usamos 'within' para limitar la búsqueda dentro de esa sección
    const withinCristopher = within(cristopherSection);

    expect(withinCristopher.getByText(/Estudiante de Ingenieria en informatica 4 semestre/i)).toBeInTheDocument();
    expect(withinCristopher.getByText(/Nuestro Scrum Master/i)).toBeInTheDocument();
    expect(withinCristopher.getByText(/Palabra clave: ESTE ES MI NUEVO POKEMON/i)).toBeInTheDocument();
