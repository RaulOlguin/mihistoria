...
// Mock de next/image
vi.mock("../componentsa/Navbar", ()=> () => <nav>Navbar</nav>);
vi.mock("next/image", () => ({
  __esModule: true,
...
describe("Verificar información cargada en Home", () => {
  ...
  test("renderiza los títulos principales", () => {
    ...
    expect(
      screen.getByRole("heading", { name: /Portada/i })
    ).toBeInTheDocument();
  });

  test("muestra la cita de Gitano Rodríguez", () => {
    expect(
      screen.getByText(/Yo no he sabido nunca de su historia/i)
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Gitano Rodriguez/i)
    ).toBeInTheDocument();
  });
  ...
  test("muestra imágenes con descripciones correctas", () => {
    expect(
      screen.getByAltText(/comparacion de calle cochrane antes y hoy/i)
    ).toBeInTheDocument();
    expect(screen.getByAltText(/restauran mural/i)).toBeInTheDocument();
    expect(
      screen.getByAltText(/voluntarios limpian plazas/i)
    ).toBeInTheDocument();
  ...
  test("muestra las tres noticias correctamente", () => {
    expect(
      screen.getByText(/Vecinos restauran mural histórico/i)
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Jornada de limpieza comunitaria/i)
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Campaña educativa sobre patrimonio/i)
    ).toBeInTheDocument();
  });
});
